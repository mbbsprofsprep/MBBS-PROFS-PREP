(function() {
    function initDeckPlugin() {
        if (!window.qnaData) {
            console.error("Deck Plugin: window.qnaData not found.");
            return;
        }
        ...
        deckLayer.innerHTML = `
            <div class="px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center shadow-sm z-10 shrink-0">
                ...
                <button onclick="window.toggleUnivDrawer('nav')" class="px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 font-bold border border-gray-200 dark:border-gray-600 dark:text-white" id="univ-deck-counter">1/1</button>
            </div>
        ...
        window.activeQuestions = [...window.qnaData];
        window.univIndex = 0;
        window.univStatus = {};
        ...
        window.renderUnivCard = function() {
            ...
            if (window.activeQuestions.length === 0) {
                container.innerHTML = `<div class="h-full flex items-center justify-center font-bold text-gray-500">No questions match filter</div>`;
                document.getElementById('univ-deck-counter').innerText = `0/0`;
                return;
            }
            ...
            let opts = q.type === 'MCQ' ? `<div class="space-y-3">${q.options.map((o,i) => {
                return `<button onclick="window.checkUnivAnswer(this, '${String.fromCharCode(65+i)}', '${q.ans_key}')" class="option-btn"><span class="w-7 h-7 inline-flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-lg mr-3">${String.fromCharCode(65+i)}</span>${o}</button>`;
            }).join('')}</div>` : `<div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl text-blue-800 dark:text-blue-300 font-medium shadow-sm border border-blue-200 dark:border-blue-800">This is a Subjective theory question. Toggle back to the Table View to ask AI for a model answer.</div>`;
            
            container.innerHTML = `<div class="deck-card"><div class="mb-3"><span class="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full text-xs font-bold border border-blue-200 dark:border-blue-800">${q.q}</span></div><h2 class="text-xl font-bold mb-6 text-gray-900 dark:text-white leading-relaxed">${q.stem}</h2>${opts}</div>`;
        };
        ...
        window.checkUnivAnswer = function(btn, picked, correct) {
            ...
            if (p === c) { 
                ...
                window.univStatus[q.q] = 'correct'; 
            } else { 
                btn.classList.add('opt-wrong'); 
                window.univStatus[q.q] = 'wrong'; 
                btn.parentElement.querySelectorAll('button').forEach((b,i) => { 
                    if(String.fromCharCode(65+i) === c) b.classList.add('opt-correct'); 
                }); 
            }
        };

        // Bulletproof View Toggling
        window.toggleUniversalMode = function() {
            window.isDeckMode = !window.isDeckMode;
            
            // This cleanly hides your original content without guessing class names
        ...
        // Bind the Click Event Safely
        fab.addEventListener('click', function(e) {
            e.preventDefault();
            window.toggleUniversalMode();
        });
        ...
        // Read Filters directly from the UI Checkboxes (Never crashes!)
        window.syncDeckWithFilters = function() {
            const searchInput = document.getElementById("searchInput");
            const search = searchInput ? searchInput.value.toLowerCase() : "";
            
            const getChecked = (menuId) => {
                const menu = document.getElementById(menuId);
                if (!menu) return [];
                return Array.from(menu.querySelectorAll('input:checked')).map(cb => cb.value);
            };

            const selectedTypes = getChecked('typeSelectMenu');
            const selectedChapters = getChecked('chapterSelectMenu');
            const selectedExams = getChecked('examSelectMenu');
            ...
            window.activeQuestions = window.qnaData.filter(q => {
                ...
                if (show && selectedExams.length > 0 && !selectedExams.includes(q.examBatch)) show = false;
        ...
        // Hook into your existing Table Filter
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initDeckPlugin);
    } else {
        initDeckPlugin();
    }
})();
